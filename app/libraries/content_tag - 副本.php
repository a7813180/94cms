...
if (!defined('BASEPATH'))
  ...
  class content_tag{
   ...
   public function _cate($arr)
	...
	if($page){
	   ...
	   return $ci->db->select('id')->get_where($tb,array('status'=>0))->num_rows();
     }
    ...
    if($arr['pagenum'] == '1' or empty($arr['pagenum'])){
	    ...
	    $zs = $ci->db->select('id')->get_where($tb,array('status'=>0))->num_rows();
 ...
 /*   if(isset($arr['sort'])){
    	if($arr['order'] == "random"){
            ...
            $ci->db->where($tb.'.id >=',"(SELECT floor(RAND() * (SELECT MAX(id) FROM `94_news`)))");
    	}else{
