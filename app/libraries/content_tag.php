...
class content_tag {
    ...
    public function _cate($arr) {
        ...
        $data = $ci->db->select($field)->order_by('sort', $arr['order'])->order_by('id', 'asc')->get_where('cate', array(
            'status ' => '1',
        ))->result_array();
    ...
    public function _list($arr, $page = null) {
        ...
        $cids[] = $arr['cid'];
        //输入cid得到模型id
        $modelid = $ci->db->select('modelid')->get_where('cate', array(
            'id' => $arr['cid']
        ))->row_array();
        //得到数据表
        $tablename = $ci->db->select('tablename')->get_where('model', array(
            'id' => $modelid['modelid']
        ))->row_array();
        //开始查询
        ...
        if ($page) {
           $ts = $ci->db->select('COUNT(*)')/*->where('status', '99')*/->where_in('cid', $cids)->limit(1)->get($tb)->row_array();
           return $ts['COUNT(*)'];
        }
        $select = $tb . ".id AS id," . $tb . ".cid AS cid," . "cate.name AS cname," . $tb . ".title AS title," . $tb . ".publishtime AS publishtime," . $tb . ".click AS click," . $tb . ".sort AS sort," . $tb . ".litpic AS litpic," . $tb . ".content AS content," . $tb . '.description AS description,' . $tb . '.shorttitle AS shorttitle,' . $tb . '.color as color,' . $tb . '.author AS author,' . $tb . '.keywords AS keywords,' . $tb . '.updatetime AS updatetime,' . "cate.ename AS ename," . "cate.type AS type," . "cate.modelid AS modelid,";
        if (!empty($arr['field'])) {
            //正则判断是不是有字段分隔符
