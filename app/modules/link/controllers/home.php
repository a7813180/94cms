...
class Link_Home_module extends CI_Module {
   ...
   public  function del()
    {
     ...
     $this->load->model('Main_data_model');
     $this->Main_data_model->del($id) ? success('删除成功!','module/'.APP_ADMIN.'/link/home') : error('删除失败');
    }
  ...
  //修改处理
   public function updata_s()
   {
    ...
    $this->db->update('link', $data, array('id' => $id)) ? success_s('修改成功','module/'.APP_ADMIN.'/link/home') : error_s('修改失败');
   }
  ...
  { 
    ...
    empty($data['url']) ? error_s('网址不可以为空!'): '';
    empty($data['sort']) ? $data['sort']=99: '';
    $this ->db ->insert('link',$data) ? success_s('添加成功','module/'.APP_ADMIN.'/link/home') : error_s('添加失败');
  }
  ...
  public function sort_s()
  {
    ...
    success('更新成功','module/'.APP_ADMIN.'/link/home');
  }
  ...
  public function del_s(){
    ...
    success("批量删除成功！",'module/'.APP_ADMIN.'/link/home');
